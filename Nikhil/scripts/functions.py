...
def load_mat_data(path_to_data, file_name, states_file, table_name="PFClfpCleaned"):
    data = loadmat(path_to_data + file_name)
    data = data[table_name].flatten()
...
def get_nrem_filtered(data, nrem_epochs, fs=2500, filter_type="iir"):
    ...
    if filter_type == "iir":
        ...
        nrem_filtered_data = filter_signal(nrem_data, fs, 'bandpass', (0.1, 4), n_cycles=3, filter_type='fir', remove_edges=False)
...
def get_filtered_epoch_data(data, epochs, band=(0.1, 4), fs=2500):
    ...
    filtered_epoch_data = filter_signal(epoch_data, fs, 'bandpass', band, n_cycles=3, filter_type='fir', remove_edges=False)
...
def get_cycles_with_metrics(cycles, data, IA, IF):
    C = copy.deepcopy(cycles)
...
# Get state for a timepoint based on sleep scoring and sampling frequency
def get_sleep_state(sample_time, sleep_scoring, fs=2500):
...
def get_delta_cycles(pfc_data, sleep_scoring, fs=1000):
    ...
    delta_cycle_data = {
        ...
        "nrem_filtered_data": nrem_filtered_data,
        "IP": IP,
        "IF": IF,
        "IP": IP,
        "cycles": cycles,
...
def rate_cycle(cycles_vector, duration=1, fs=1000):
    ...
    segments = np.array_split(
        cycles_vector,
        np.arange(samples_per_segment, len(cycles_vector), samples_per_segment),
    )
